...
//# BEGIN TODO: Functional decomposition; auxiliary method(s)
     ...
     * (checks all rows in both keys)
     * @param key1 the first key
     * @param key2 the second key
     * @return {@code true} if {@code key1} can be converted into {@code key2} or
     * {@code false} if it can not
    ...
    boolean CK(int[][] key1, int[][] key2)
    {
        for (int i = 0; i < key1.length; i ++) {
            if (!CR(key1[i], key2[i])) { // checking rows in keys
                return false;
            }
        }
        return true;
     ...
     * @return {@code true} if {@code row1} CAN NOT be converted into {@code row2} or
     * {@code true} if it can not
